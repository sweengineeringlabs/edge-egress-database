...
pub(crate) async fn connect(cfg: &DatabaseConfig) -> Result<DbPool, MigrationError> {
    match cfg.driver {
        #[cfg(feature = "sqlite")]
        DriverKind::Sqlite => Ok(DbPool::Sqlite(build_sqlite_pool(cfg).await?)),
        #[cfg(feature = "postgres")]
        DriverKind::Postgres => Ok(DbPool::Postgres(build_pg_pool(cfg).await?)),
...
/// Open a pool for `cfg`, run pending migrations from `cfg.migrations_dir`, and
/// return the ready pool.
pub(crate) async fn connect_and_migrate(cfg: &DatabaseConfig) -> Result<DbPool, MigrationError> {
...
pub(crate) async fn run_migrations(
    ...
    match pool {
        #[cfg(feature = "sqlite")]
        DbPool::Sqlite(p) => migrator.run(p).await.map_err(to_apply_error)?,
        #[cfg(feature = "postgres")]
        DbPool::Postgres(p) => migrator.run(p).await.map_err(to_apply_error)?,
    };
...
/// Versions recorded in `_sqlx_migrations`. Absent table (no migration has run
/// yet) yields an empty set rather than an error.
